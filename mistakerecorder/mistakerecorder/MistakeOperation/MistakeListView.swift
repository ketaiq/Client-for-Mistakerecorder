                    ...
                    HStack {
                        NavigationLink(destination: MistakeItemView(user: user, mistake: mistake).navigationBarTitleDisplayMode(.inline)) {
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                Text("\(DateFunctions.functions.getYearFromDate(givenDate: mistake.createdDate).description)年\(DateFunctions.functions.getMonthFromDate(givenDate: mistake.createdDate))月\(DateFunctions.functions.getDayFromDate(givenDate: mistake.createdDate))日")
                                    .font(.system(size: 18))
                            }
                            .padding(.vertical)
                        }
